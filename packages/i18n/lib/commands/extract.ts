  ...
  async extract(opts: {logger: Logger}) {
    ...
    const getSourceMapper = async () => {
      ...
      if (!existsSync(sourcemapperPath)) {
        opts.logger.reportInfo(MessageName.UNNAMED, "Cloning https://github.com/dhoko/sourcemapper.git");
        await execAsync(`git clone --quiet --single-branch --depth 1 https://github.com/dhoko/sourcemapper.git ${sourcemapperPath}`);
      } else {
        opts.logger.reportInfo(MessageName.UNNAMED, "Using cached sourcemapper");
      }
      ...
      if (platform === "darwin") {
        ...
        opts.logger.reportError(MessageName.ERROR, "Windows platform is not supported");
      } else {
    ...
    const getDistDirectory = async () => {
      ...
      if (existsSync("dist")) {
        opts.logger.reportInfo(MessageName.UNNAMED, "Extracting from local dist bundle");
        return "dist";
      }
    ...
    const packingBundle = async () => {
      await opts.logger.startProgressAsync(Logger.progressViaTitle(), async progress => {
        ...
        const getBundle = async (file: string) => {
          ...
          await execAsync(`${sourcemapperBinary} --input ${file} --output 'i18n-js'`);
        };
    ...
    const linkingTemplate = async () => {
      await execAsync(`cd i18n-js && ../../../node_modules/.bin/ttag extract . -o "../template.pot"`);
      await execAsync(`rm -rf "./i18n-js"`);
    };

    await opts.logger.startTimerAsync("Getting SourceMapper step", {
      skipIfEmpty: false,
    }, async () => {
    ...
    await opts.logger.startTimerAsync("Bandling step", {
      skipIfEmpty: false,
    }, async () => {
      dist = await getDistDirectory();
    });

    await opts.logger.startTimerAsync("Packing step", {
      skipIfEmpty: false,
    }, async () => {
      await packingBundle();
    });

    await opts.logger.startTimerAsync("Link step", {
      skipIfEmpty: false,
    }, async () => {
